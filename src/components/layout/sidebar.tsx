...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        'relative flex h-full flex-col transition-all duration-300 ease-in-out border-r flex-shrink-0',
    ...
    >
      ...
      {/* Logo/Brand */}
      <div className={cn('flex h-16 shrink-0 items-center px-4 border-b border-sidebar-border', collapsed ? 'justify-center' : 'gap-3')}>
        <Link href="/dashboard" className="flex items-center gap-3">
          ...
          {!collapsed && (
            <span className="text-lg font-black tracking-tight text-sidebar-foreground" style={{ fontFamily: 'Outfit, sans-serif' }}>
              Kings EC
            </span>
          )}
        </Link>
      ...
      <button
        ...
        className="absolute -right-3 top-[72px] z-20 flex h-6 w-6 items-center justify-center rounded-full bg-secondary border border-border shadow-md text-muted-foreground hover:text-foreground hover:bg-secondary/80 transition-all duration-200"
        ...
        {visibleNav.map((item) => {
          ...
          return (
            ...
            >
              ...
              {collapsed && (
                <div className="pointer-events-none absolute left-full ml-3 px-2.5 py-1.5 bg-popover border border-border text-foreground text-xs font-medium rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-xl z-50">
                  {item.name}
                </div>
              )}
            </Link>
      ...
      {/* User profile bottom — real data from useUser */}
      {!collapsed && (
