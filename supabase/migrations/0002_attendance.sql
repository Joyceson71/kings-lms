...
CREATE POLICY "Faculty can read their course sessions" ON public.course_sessions FOR SELECT USING (
    ...
    EXISTS (SELECT 1 FROM public.courses WHERE id = public.course_sessions.course_id AND created_by = auth.uid())
);
...
CREATE POLICY "Students can read active course sessions" ON public.course_sessions FOR SELECT USING (
    status = 'active' AND
    EXISTS (SELECT 1 FROM public.course_enrollments WHERE course_id = public.course_sessions.course_id AND student_id = auth.uid())
);
...
CREATE POLICY "Faculty can read attendance logs" ON public.attendance_logs FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.course_sessions WHERE id = public.attendance_logs.session_id AND created_by = auth.uid())
);
...
CREATE POLICY "Students can mark attendance" ON public.attendance_logs FOR INSERT WITH CHECK (
    ...
    EXISTS (
        ...
        WHERE id = public.attendance_logs.session_id 
        AND status = 'active'
    )
);
